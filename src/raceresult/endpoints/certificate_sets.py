"""CertificateSets endpoint for Raceresult API."""

from __future__ import annotations

from typing import TYPE_CHECKING, Any

from raceresult.models.certificate_set import CertificateSet

if TYPE_CHECKING:
    from raceresult.client import RaceResultClient


class CertificateSetsEndpoint:
    """Certificate sets API endpoint."""

    def __init__(self, client: RaceResultClient, event_id: str):
        self._client = client
        self._event_id = event_id

    async def names(self) -> list[str]:
        """Return names of all certificate sets."""
        result = await self._client.get_json(self._event_id, "certificatesets/names")
        return result if result else []

    async def get(self, name: str) -> CertificateSet:
        """Return a certificate set by name."""
        result = await self._client.get_json(
            self._event_id, "certificatesets/get", {"name": name}
        )
        return CertificateSet.model_validate(result)

    async def save(self, item: CertificateSet) -> None:
        """Save a certificate set."""
        await self._client.post_json(
            self._event_id, "certificatesets/save", data=item.model_dump(by_alias=True)
        )

    async def delete(self, name: str) -> None:
        """Delete a certificate set."""
        await self._client.get(self._event_id, "certificatesets/delete", {"name": name})

    async def copy(self, name: str, new_name: str) -> None:
        """Create a copy of a certificate set."""
        await self._client.get(
            self._event_id, "certificatesets/copy", {"name": name, "newName": new_name}
        )

    async def rename(self, name: str, new_name: str) -> None:
        """Rename a certificate set."""
        await self._client.get(
            self._event_id, "certificatesets/rename", {"name": name, "newName": new_name}
        )

    async def new(self, name: str) -> None:
        """Create a new certificate set."""
        await self._client.get(self._event_id, "certificatesets/new", {"name": name})

    async def create(
        self,
        name: str,
        contests: list[int] | None = None,
        filter_expr: str = "",
        lang: str = "",
    ) -> bytes:
        """Render a certificate set as PDF."""
        params: dict[str, Any] = {"name": name, "filter": filter_expr, "lang": lang}
        if contests:
            params["contest"] = ",".join(str(c) for c in contests)
        return await self._client.get(self._event_id, "certificatesets/create", params)

    async def count(self, name: str, contests: list[int] | None = None) -> int:
        """Return the number of participants included in a certificate set."""
        params: dict[str, Any] = {"name": name}
        if contests:
            params["contest"] = ",".join(str(c) for c in contests)
        result = await self._client.get_json(self._event_id, "certificatesets/count", params)
        return int(result) if result is not None else 0
