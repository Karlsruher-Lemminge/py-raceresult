...
class TestKiosk:
    ...
    def test_null_steps(self):
        """API returns null instead of [] when there are no steps."""
        assert Kiosk.model_validate({"Name": "x", "Steps": None}).steps == []
...
class TestGroupTimes:
    ...
    def test_null_items(self):
        """API returns null instead of [] when there are no items."""
        assert GroupTimes.model_validate({"Mode": "wave", "Items": None}).items == []
...
class TestWebHook:
    ...
    def test_null_fields(self):
        """API returns null instead of [] when there are no fields."""
        assert WebHook.model_validate({"Name": "h", "Fields": None}).fields == []
...
class TestArchivesModels:
    ...
    def test_participant_null_participations(self):
        """API returns null instead of [] when there is no history."""
        assert ArchivesParticipant.model_validate(
            {"Lastname": "Müller", "Participations": None}
        ).participations == []
...
class TestCertificate:
    ...
    def test_certificate_null_fields_and_zones(self):
        """API returns null instead of [] for Fields/Zones on a bare template."""
        cert = Certificate.model_validate(
            {"CertificateName": "Empty", "Fields": None, "Zones": None}
        )
        assert cert.elements == []
        assert cert.zones == []
